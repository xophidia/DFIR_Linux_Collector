...
#search Account with Chromium Profile
browser="chromium"
homepaths=$(find /home \( -name "chromium" \)| awk -F"/" '{print "/"$2"/"$3"/"}'| uniq)
chromiumfiles=( "addons.json" "containers.json" )
if [[ -n $homepaths ]]; then
        for homepath in $homepaths; do
                username=$(echo $homepath | cut -d"/" -f3)
                #output_path="OUTPUT/Browser/${username}/Chromium"
                output_path="${OUTPUT}/Browser/${username}/Chromium"
                for file in "${chromiumfiles[@]}"; do
		...
		#HISTORY
			...
			if [[ -n $gethistory ]]; then
				echo "{\"Chromium History\": " >> $output_path/history.json
			...
			if [[ -n $getdownloads ]]; then
				echo "{\"Chromium Downloads\": " >> $output_path/downloads.json
		...
		#Cookies
			...
			if [[ -n $getcookies ]]; then
				echo "{\"Chromium Cookies\": " >> $output_path/cookies.json
